...
// TestUpgrade tests if an Blackfury node can be upgraded from one version to another.
...
// TestCLITxs executes different types of transactions against an Blackfury node
	...
	}{
		{
			...
			cmd: func() (string, error) {
				return s.upgradeManager.CreateSubmitProposalExec(
					...
					5000,
					true,
					"--fees=5000000000ablackfury",
					"--gas-prices=50000ablackfury",
				)
			},
			expPass:   false,
		...
		},
		{
			...
			cmd: func() (string, error) {
				return s.upgradeManager.CreateSubmitProposalExec(
					...
					5000,
					true,
					"--gas-prices=1000000000ablackfury",
					"--gas=1500000",
				)
			},
			expPass: true,
