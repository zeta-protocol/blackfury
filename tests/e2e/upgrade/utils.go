...
// BlackfuryVersions is a custom comparator for sorting semver version strings.
type BlackfuryVersions []string

// Len is the number of stored versions..
func (v BlackfuryVersions) Len() int { return len(v) }

// Swap swaps the elements with indexes i and j. It is needed to sort the slice.
func (v BlackfuryVersions) Swap(i, j int) { v[i], v[j] = v[j], v[i] }

// Less compares semver versions strings properly
func (v BlackfuryVersions) Less(i, j int) bool {
...
func CheckLegacyProposal(version string) bool {
	...
	cmp := BlackfuryVersions([]string{version, "v10.0.0"})
...
func RetrieveUpgradesList(upgradesPath string) ([]string, error) {
	...
	sort.Sort(BlackfuryVersions(versions))
