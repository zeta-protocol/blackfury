...
Blackfury uses [Tendermint’s coding repo](https://github.com/tendermint/coding)
...
1. Either [open](https://github.com/zeta-protocol/blackfury/issues/new/choose)
   or [find](https://github.com/zeta-protocol/blackfury/issues) an issue you have identified and would like to contribute to
   resolving.
...
3. If you would like to contribute:
    ...
    8. Please make sure to run `make format` before every commit -
       ...
       There are CI tests built into the Blackfury repository
...
Check out our [good first issues](https://github.com/zeta-protocol/blackfury/issues?q=label%3A%22good+first+issue%22).

## <span id="adr">Architecture Decision Records (ADR)</span>

When proposing an architecture decision for Blackfury,
please create an [ADR](https://github.com/zeta-protocol/blackfury/blob/main/docs/architecture/README.md)
...
While my fork lives at `https://github.com/zeta-protocol/blackfury`,
the code should never exist at `$GOPATH/src/github.com/zeta-protocol/blackfury`.
Instead, we use `git remote` to add the fork as a new remote for the original repo,`$GOPATH/src/github.com/zeta-protocol/blackfury`,
...
2. Go to the original repo checked out locally. (i.e. `$GOPATH/src/github.com/zeta-protocol/blackfury`)
3. `git remote rename origin upstream`
4. `git remote add origin git@github.com:tharsis/blackfury.git`

Now `origin` refers to my fork and `upstream` refers to the blackfury version.
So I can `git push -u origin master` to update my fork,
and make pull requests to blackfury from there.
...
4. Create your feature branch from `development` either on `github.com/zeta-protocol/blackfury`, or your fork (
...
**Note**: tharsis’s Blackfury team currently cuts releases on a need to have basis.
