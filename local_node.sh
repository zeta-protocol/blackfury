...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
	...
	jq '.app_state["staking"]["params"]["bond_denom"]="ablackfury"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	jq '.app_state["crisis"]["constant_fee"]["denom"]="ablackfury"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	jq '.app_state["gov"]["deposit_params"]["min_deposit"][0]["denom"]="ablackfury"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	jq '.app_state["evm"]["params"]["evm_denom"]="ablackfury"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	jq '.app_state["inflation"]["params"]["mint_denom"]="ablackfury"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	...
	jq -r --arg amount_to_claim "$amount_to_claim" '.app_state["bank"]["balances"] += [{"address":"blackfury15cvq3ljql6utxseh0zau9m8ve2j8erz89m5wkz","coins":[{"denom":"ablackfury", "amount":$amount_to_claim}]}]' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	...
	for KEY in "${KEYS[@]}"; do
		blackfuryd add-genesis-account "$KEY" 100000000000000000000000000ablackfury --keyring-backend $KEYRING --home "$HOMEDIR"
	done
	...
	blackfuryd gentx "${KEYS[0]}" 1000000000000000000000ablackfury --keyring-backend $KEYRING --chain-id $CHAINID --home "$HOMEDIR"
	...
	## 3. Clone this ~/.blackfuryd home directory into some others, let's say `~/.clonedBlackfuryd`
	## 4. Run `gentx` in each of those folders
	## 5. Copy the `gentx-*` folders under `~/.clonedBlackfuryd/config/gentx/` folders into the original `~/.blackfuryd/config/gentx`
...
blackfuryd start --metrics "$TRACE" --log_level $LOGLEVEL --minimum-gas-prices=0.0001ablackfury --json-rpc.api eth,txpool,personal,net,debug,web3 --api.enable --home "$HOMEDIR"
