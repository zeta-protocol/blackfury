...
HTTPS_GIT := https://github.com/zeta-protocol/blackfury.git
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/zeta-protocol/blackfury"
	godoc -http=:6060
...
localnet-start: localnet-stop localnet-build
	@if ! [ -f build/node0/$(BLACKFURY_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/blackfury:Z blackfury/node "./blackfuryd testnet init-files --v 4 -o /blackfury --keyring-backend=test --starting-ip-address 192.167.10.2"; fi
	docker-compose up -d
...
PACKAGE_NAME:=github.com/zeta-protocol/blackfury
