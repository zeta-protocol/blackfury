...
import (
	...
	"github.com/zeta-protocol/blackfury/v13/x/inflation/types"
)
...
func (suite *KeeperTestSuite) TestGetCirculatingSupplyAndInflationRate() {
	...
	bondedCoins := sdk.NewDecCoin(blackfurytypes.AttoBlackfury, bondedAmt)
	...
	testCases := []struct {
	...
	}{
		{
			...
			sdk.TokensFromConsensusPower(400_000_000, blackfurytypes.PowerReduction).Sub(bondedAmt),
			func() {
		...
		{
			"high supply",
			sdk.TokensFromConsensusPower(800_000_000, blackfurytypes.PowerReduction).Sub(bondedAmt),
			func() {},
		...
		},
		{
			...
			sdk.TokensFromConsensusPower(200_000_000, blackfurytypes.PowerReduction).Sub(bondedAmt),
			func() {},
			sdk.ZeroDec(),
		},
	}
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			teamAlloc := sdk.NewDecCoin(
				types.DefaultInflationDenom,
				sdk.TokensFromConsensusPower(int64(200_000_000), blackfurytypes.PowerReduction),
			)
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			if tc.isMainnet {
				...
				suite.ctx = suite.ctx.WithChainID("blackfury_9999-666")
			}
			tc.malleate()
