...
import (
	...
	"github.com/zeta-protocol/blackfury/v13/server/config"
...
func (suite *KeeperTestSuite) SetupIBCTest() {
	...
	s.app = suite.BlackfuryChain.App.(*app.Blackfury)
	evmParams := s.app.EvmKeeper.GetParams(s.BlackfuryChain.GetContext())
	evmParams.EvmDenom = utils.BaseDenom
	err := s.app.EvmKeeper.SetParams(s.BlackfuryChain.GetContext(), evmParams)
	suite.Require().NoError(err)

	// s.app.FeeMarketKeeper.SetBaseFee(s.BlackfuryChain.GetContext(), big.NewInt(1))
	...
	validators := s.app.StakingKeeper.GetValidators(suite.BlackfuryChain.GetContext(), 2)
	cons, err := validators[0].GetConsAddr()
	suite.Require().NoError(err)
	suite.BlackfuryChain.CurrentHeader.ProposerAddress = cons.Bytes()

	err = s.app.StakingKeeper.SetValidatorByConsAddr(suite.BlackfuryChain.GetContext(), validators[0])
	suite.Require().NoError(err)

	_, err = s.app.EvmKeeper.GetCoinbaseAddress(suite.BlackfuryChain.GetContext(), sdk.ConsAddress(suite.BlackfuryChain.CurrentHeader.ProposerAddress))
	suite.Require().NoError(err)
	...
	err = s.app.BankKeeper.MintCoins(suite.BlackfuryChain.GetContext(), inflationtypes.ModuleName, coins)
	suite.Require().NoError(err)
	err = s.app.BankKeeper.SendCoinsFromModuleToAccount(suite.BlackfuryChain.GetContext(), inflationtypes.ModuleName, suite.BlackfuryChain.SenderAccount.GetAddress(), coins)
	suite.Require().NoError(err)
	...
	err = s.app.BankKeeper.MintCoins(s.BlackfuryChain.GetContext(), types.ModuleName, coins)
	...
	err = s.app.ClaimsKeeper.SetParams(suite.BlackfuryChain.GetContext(), claimparams)
	suite.Require().NoError(err)

	params := types.DefaultParams()
	params.EnableErc20 = true
	err = s.app.Erc20Keeper.SetParams(suite.BlackfuryChain.GetContext(), params)
	suite.Require().NoError(err)
	...
	coinBlackfury = sdk.NewCoin(utils.BaseDenom, sdk.NewInt(1000000000000000000))
	coins = sdk.NewCoins(coinBlackfury)
	err = s.app.BankKeeper.MintCoins(suite.BlackfuryChain.GetContext(), types.ModuleName, coins)
	suite.Require().NoError(err)
	err = s.app.BankKeeper.SendCoinsFromModuleToModule(suite.BlackfuryChain.GetContext(), types.ModuleName, authtypes.FeeCollectorName, coins)
	suite.Require().NoError(err)
}
...
// to the Blackfury chain (used on IBC tests)
func (suite *KeeperTestSuite) DeployContractToChain(name, symbol string, decimals uint8) (common.Address, error) {
	return testutil.DeployContract(
		s.BlackfuryChain.GetContext(),
		s.BlackfuryChain.App.(*app.Blackfury),
		suite.BlackfuryChain.SenderPrivKey,
		suite.queryClientEvm,
		contracts.ERC20MinterBurnerDecimalsContract,
		name, symbol, decimals,
	)
...
// Send back coins (from path endpoint B to A). In case of IBC coins need to provide ibcCoinMetadata (<port>/<channel>/<denom>, e.g.: "transfer/channel-0/ablackfury") as input parameter.
