...
type KeeperTestSuite struct {
	...
	pathOsmosisBlackfury  *ibctesting.Path
	pathCosmosBlackfury   *ibctesting.Path
...
var (
	s *KeeperTestSuite
	// sendAndReceiveMsgFee corresponds to the fees paid on Blackfury chain when calling the SendAndReceive function
	// This function makes 3 cosmos txs under the hood
	sendAndReceiveMsgFee = sdk.NewInt(ibctesting.DefaultFeeAmt * 3)
	// sendBackCoinsFee corresponds to the fees paid on Blackfury chain when calling the SendBackCoins function
	// or calling the SendAndReceive from another chain to Blackfury
