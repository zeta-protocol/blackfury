...
var _ = Describe("Recovery: Performing an IBC Transfer", Ordered, func() {
	...
	Describe("from a non-authorized chain", func() {
		BeforeEach(func() {
			...
			err := s.BlackfuryChain.App.(*app.Blackfury).ClaimsKeeper.SetParams(s.BlackfuryChain.GetContext(), params)
		...
		It("should transfer and not recover tokens", func() {
			...
			ibcOsmo := s.BlackfuryChain.App.(*app.Blackfury).BankKeeper.GetBalance(s.BlackfuryChain.GetContext(), receiverAcc, teststypes.UosmoIbcdenom)
		...
		Describe("to the sender's own eth_secp256k1 account on Blackfury (sender == recipient)", func() {
			...
			Context("with disabled recovery parameter", func() {
				BeforeEach(func() {
					...
					s.BlackfuryChain.App.(*app.Blackfury).RecoveryKeeper.SetParams(s.BlackfuryChain.GetContext(), params) //nolint:errcheck
				})
			...
			Context("with a sender's claims record", func() {
				Context("without completed actions", func() {
					BeforeEach(func() {
						...
						err := testutil.FundModuleAccount(s.BlackfuryChain.GetContext(), s.BlackfuryChain.App.(*app.Blackfury).BankKeeper, claimstypes.ModuleName, coins)
						s.Require().NoError(err)
					})
				...
				Context("with completed actions", func() {
					...
					BeforeEach(func() {
						amt := sdk.NewInt(int64(100))
						coins := sdk.NewCoins(sdk.NewCoin("ablackfury", sdk.NewInt(int64(75))))
						claim = claimstypes.NewClaimsRecord(amt)
					...
					It("should transfer tokens to the recipient and perform recovery", func() {
						...
						ibcBlackfury := s.IBCOsmosisChain.GetSimApp().BankKeeper.GetBalance(s.IBCOsmosisChain.GetContext(), receiverAcc, teststypes.AblackfuryIbcdenom)
						Expect(ibcBlackfury).To(Equal(sdk.NewCoin(teststypes.AblackfuryIbcdenom, coinBlackfury.Amount)))
			...
			Context("without a sender's claims record", func() {
				...
				When("recipient has additional ibc vouchers that originated from other chains", func() {
					...
					It("should not recover tokens that originated from other chains", func() {
						...
						ibcAtom := s.BlackfuryChain.App.(*app.Blackfury).BankKeeper.GetBalance(s.BlackfuryChain.GetContext(), senderAcc, teststypes.UatomIbcdenom)
				...
				When("recipient has additional non-native ibc vouchers that originated from senders chains", func() {
					BeforeEach(func() {
						params := types.DefaultParams()
						params.EnableRecovery = false
						err := s.BlackfuryChain.App.(*app.Blackfury).RecoveryKeeper.SetParams(s.BlackfuryChain.GetContext(), params)
						s.Require().NoError(err)
						...
						transferMsg := transfertypes.NewMsgTransfer(s.pathOsmosisBlackfury.EndpointA.ChannelConfig.PortID, s.pathOsmosisBlackfury.EndpointA.ChannelID, sdk.NewCoin(teststypes.UatomIbcdenom, sdk.NewInt(10)), sender, receiver, timeoutHeight, 0, "")
						...
						osmoIBCAtom := s.BlackfuryChain.App.(*app.Blackfury).BankKeeper.GetBalance(s.BlackfuryChain.GetContext(), receiverAcc, teststypes.UatomOsmoIbcdenom)
